...
class ViewTest : public ::testing::Test {
  protected:
    ...
    Field field(std::u8string_view member, std::u8string_view name) const {
...
TEST_F(ViewTest, eventTarget) {
    ...
    data_->view_change_event[fieldBase("a", "b")]->operator()(field("a", "b"));
...
TEST_F(ViewTest, viewSet) {
    ...
    EXPECT_EQ(view_data[5].text_, u8"f");
    EXPECT_EQ(view_data[5].on_click_func_->member_, self_name);
    EXPECT_EQ(view_data[5].on_click_func_->field_, u8"f");

    EXPECT_EQ(view_data[6].type_, ViewComponentType::button);
    EXPECT_EQ(view_data[6].text_, u8"a");
    ...
    EXPECT_EQ(view_data[7].text_, u8"a2");
    ...
    EXPECT_EQ(view_data[8].text_, u8"i");
    ...
    EXPECT_EQ(view_data[9].text_, u8"i2");
    ...
    EXPECT_EQ(view_data[10].text_, u8"i3");
    ...
    EXPECT_EQ((*data_->view_store.getRecv(self_name, u8"b"))->size(), 0);
    ...
    EXPECT_EQ((*data_->view_store.getRecv(self_name, u8"b"))->size(), 1);
...
TEST_F(ViewTest, viewGet) {
    ...
    auto vd = std::make_shared<std::vector<ViewComponentBase>>(
        std::vector<ViewComponentBase>{
            Components::text("a").toV().lockTmp(data_, u8"b", &idx_next),
            Components::button("a", [] {}).lockTmp(data_, u8"b", &idx_next),
            Components::text("a").toV().lockTmp(data_, u8"b", &idx_next),
        });
    data_->view_store.setRecv(u8"a", u8"b", vd);
    ...
    EXPECT_EQ(components.at(0).id(), u8"..0.0"); // type0, idx0
    EXPECT_EQ(components.at(1).type(), ViewComponentType::button);
    EXPECT_EQ(components.at(1).text(), "a");
    EXPECT_EQ(components.at(1).id(), u8"..2.0"); // type2, idx0
    ASSERT_TRUE(components.at(1).onClick().has_value());
    EXPECT_EQ(components.at(1).onClick()->member().name(),
              Encoding::decode(self_name));
    ...
    EXPECT_EQ(components.at(2).id(), u8"..0.1"); // type0, idx1

    EXPECT_EQ(view("a", "c").tryGet(), std::nullopt);
    EXPECT_EQ(view("a", "c").get().size(), 0);

    EXPECT_EQ(data_->view_store.transferReq().at(u8"a").at(u8"b"), 1);
    EXPECT_EQ(data_->view_store.transferReq().at(u8"a").at(u8"c"), 2);
    ...
    EXPECT_EQ(data_->view_store.transferReq().at(u8"a").at(u8"d"), 3);
}
