...
TEST_F(CClientTest, funcRun) {
    ...
    std::thread t([&]() {
        ...
        EXPECT_EQ(wcfFuncFreeResult(async_ret), WCF_BAD_HANDLE);
        EXPECT_EQ(wcfFuncWaitResult(async_res, &async_ret), WCF_NOT_FOUND);
        EXPECT_EQ(wcfFuncFreeResult(async_ret), WCF_OK);
        ...
        EXPECT_EQ(wcfFuncFreeResult(ret), WCF_OK);
        EXPECT_EQ(wcfFuncFreeResult(ret), WCF_BAD_HANDLE);
        wcfFuncRunAsync(wcli_, "a", "b", args, 3, &async_res);
