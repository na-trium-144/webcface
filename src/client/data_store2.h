 ...
 */
template <typename T> class SyncDataStore2 {
  std::mutex mtx;
  //! 次のsend時に送信するデータ。
   ...
   */
  std::unordered_map<std::string, std::unordered_map<std::string, T>> data_recv;
  //! 受信済みのentry
...
public:
  ...
  void setRecv(const std::string &from, const std::string &name, const T &data);
