...
TEST_F(DataTest, valueSetVec) {
    ...
    d6.push_back(2);
    d6.push_back(3);
    EXPECT_THROW(d6[3].set(4), std::out_of_range);
    EXPECT_FALSE(d6[3].tryGet().has_value());
    d6.resize(4);
    EXPECT_EQ(d6.getVec(), (std::vector<double>{1, 2, 3, 0}));
    EXPECT_EQ(d6[3].tryGet().value(), 0);
    d6[3].set(4);
    EXPECT_EQ(d6[3].tryGet().value(), 4);
    EXPECT_THROW(d6.at(4).set(5), std::out_of_range);
    EXPECT_FALSE(d6.at(4).tryGet().has_value());
    d6.resize(5);
    EXPECT_EQ(d6.getVec(), (std::vector<double>{1, 2, 3, 4, 0}));
    EXPECT_EQ(d6.at(4).tryGet().value(), 0);
    d6.at(4).set(5);
    EXPECT_EQ(d6.at(4).tryGet().value(), 5);
...
TEST_F(DataTest, valueGet) {
    data_->value_store.setRecv("a"_ss, "b"_ss, MutableNumVector{123});
