...
#pragma GCC diagnostic ignored "-Wswitch"
        ...
        case MessageKind::sync_init: {
            ...
            if (v.member == "") {
                ...
                for (const auto &c : store.clients) {
                    if (c.second->sync_init && c.second->name != this->name &&
                        this->name != "") {
                        c.second->send(WebCFace::Message::packSingle(v));
                    }
                }
            }
            ...
            for (const auto &c : store.clients) {
                if (c.second->sync_init && c.second->name != this->name &&
                    ...
                    WebCFace::Message::pack(buffer,len, 
                    ...
                    for (const auto &p : c.second->func) {
                        WebCFace::Message::pack(buffer, len, p.second);
                    }

                    this->send(WebCFace::Message::packDone(buffer, len));
                }
            }
            break;
        }
        case MessageKind::call: {
            ...
            if (c_it != store.clients_by_name.end()) {
                c_it->second->send(WebCFace::Message::packSingle(v));
            } else {
                // 関数存在しないときの処理
                this->send(WebCFace::Message::packSingle(
        ...
        case MessageKind::value: {
            ...
            if (!this->value.count(v.field)) {
                for (const auto &c : store.clients) {
                    if (c.second->sync_init && c.second->name != this->name) {
                        c.second->send(WebCFace::Message::packSingle(
                            WebCFace::Message::Entry<WebCFace::Message::Value>{
                                {}, v.member, v.field}));
                    }
                }
        ...
        case kind_req(MessageKind::value): {
            ...
            if (c_it != store.clients_by_name.end()) {
                ...
                if (it != c_it->second->value.end()) {
                    this->send(
                        WebCFace::Message::packSingle(WebCFace::Message::Value{
        ...
        case kind_req(MessageKind::text): {
            ...
            if (c_it != store.clients_by_name.end()) {
                ...
                if (it != c_it->second->text.end()) {
                    this->send(
                        WebCFace::Message::packSingle(WebCFace::Message::Text{
                            {}, s.member, s.field, it->second}));
                }
            }
            break;
        ...
        case kind_req(MessageKind::view): {
            ...
            if (c_it != store.clients_by_name.end()) {
                ...
                if (it != c_it->second->view.end()) {
                    ...
                    this->send(
                        WebCFace::Message::packSingle(WebCFace::Message::View{
        ...
        case MessageKind::log_req: {
            ...
            std::cout << this->name << ": subscribe log " << s.member
                      << std::endl;
